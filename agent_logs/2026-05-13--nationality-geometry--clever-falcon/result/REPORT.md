...
- ![chord steering — full-residual anchor at the canonical layer 28](figures/chord_steering_L28.png) — α∈{0,1}, all five triples: P(A) (red) stays dominant and only mildly eroded, P(C) (blue) barely lifts off zero, P(B) (green) never emerges. Representative of the L20–L31 sweep (`chord_steering.json`); the no-flip conclusion holds at every layer.
- **Interpretation**: Stage A confirms the map is geometrically metric *as a cached representation*. Stage B shows that property is **not causally exploitable at the last-token position at any depth**: even a 100% transplant of the entire last-token residual at layer 31 — one block from the unembed — cannot redirect the answer. The monotone rise of P_C with hook depth is the fingerprint of **attention reconstruction**: the layer hook leaves ≥1 attention block downstream which, at the last position, re-reads the un-swapped *question* tokens and rebuilds answer A. The relational answer is therefore **position-distributed and attention-reconstructed from the question tokens, not carried by the last-token residual** — a direct, depth-resolved instance of the decodable-≠-causal gap. The betweenness/steering ("B at the midpoint") hypothesis is **not testable at last_token** under linear interchange; §6.7 then shows the framework's *geodesic* manifold method is structurally inapplicable to this categorical-relational task. Net: the §6.2–§6.5 geometry claim stands as a *readout* result and is now precisely bounded — the map is decodable, not the causal substrate of the answer token.
- **Artifacts**: `result/chord_betweenness.json` (Stage A); `result/chord_steering.json` (Stage B, full L20–L31 sweep) + `result/figures/chord_steering_L28.png` (canonical-layer panel transported back); `code/analyses/chord_betweenness.py`, `code/analyses/chord_steering.py`. Computed on RunPod, transported via the lightweight git path.

### 6.7 framework manifold/path-steering — structural non-applicability ("did you run the actual method?")

- **Research question**: §6.6 ran *linear* interchange (a bounded-negative). The framework's headline method is **geodesic steering through a fitted activation manifold** (`path_steering`, `path_modes: [geometric, linear]`) — linear is its degenerate special case. Does the framework's actual method move the relational answer where linear does not?
- **What was attempted**: the native pipeline `subspace(PCA-32) → activation_manifold → path_steering`, at the §6 site (L28, `last_token`), wired as a `country_borders` runner mirroring the 2-D `graph_walk` precedent. Subspace ran cleanly on RunPod (PCA 53.2% / 22.0% / 8.7% — the §6.2 lat/lon structure). `activation_manifold` could not fit a 2-D manifold under **either** shipped method:
  - **`spline`** → `ValueError: intrinsic_dim=2 but got 1 parameters. For spline method, intrinsic_dim must match number of causal parameters.` The TPS manifold is parameterized by the task's *causal parameters*; `country` is a single categorical variable, so spline is structurally locked to a meaningless 1-D ordering of 30 countries (`causalab/methods/spline/train.py`).
  - **`flow`** → `TypeError: train_manifold() got an unexpected keyword argument 'features'`. Beyond the signature mismatch, `flow.train.train_manifold` re-collects features by running the model, but `activation_manifold` deliberately loads a **weightless lite pipeline** on *cached* features; flow is also not Hydra-wired (hardcoded 10k-step `ManifoldTrainConfig`, an Invariant-5 gap) and no shipped runner exercises it. The flow manifold path is incomplete for this analysis.
- **Interpretation (the contribution)**: this is the principled answer to "did you try the actual method?" — *yes, and the attempt is itself the result.* The framework's manifold/path-steering machinery **presupposes a task parameterized by a metric causal variable** — scalar (months/weekdays) or coordinate-vector (`graph_walk`'s `node_coordinates`). `country_borders` is **categorical-relational**: the Europe geometry is *latent in the activations* (§6.2's PCA), not a task parameter. So geodesic steering is **structurally out of scope for this task as parameterized** — a different statement from §6.6's empirical linear bounded-negative, and a sharper, methodology-level finding (decodable geometry without a metric causal parameterization is not a manifold the framework can traverse). The only route to a positive geodesic test is to re-parameterize the task by geographic coordinates (a `capital_coordinates` causal variable mirroring `graph_walk`) — scoped as deliberate future work (§10), not a dead end.
- **Artifacts**: RunPod tracebacks (above); evidence in-tree at `causalab/methods/spline/train.py` (the `intrinsic_dim == n_params` constraint) and the `causalab/methods/flow/train.py` vs `causalab/analyses/activation_manifold/fitting_pipeline.py` signature mismatch. The runner config used to surface this was reverted to keep the PR a clean task-only addition (commit log).
...
- **Map decodable; steering bounded-negative (linear) and structurally out-of-scope (geodesic)** (§6.6–§6.7): the geometry result is a *readout* claim. Stage A shows the manifold is metric as a cached representation; §6.6's full-residual linear interchange (L20–L31) cannot redirect the answer (attention-reconstructed from the question tokens); §6.7 shows the framework's geodesic manifold-steering method is structurally inapplicable because `country_borders` has no metric causal parameter (the geometry is latent, not parameterized). A positive geodesic test would require re-parameterizing the task by geographic coordinates (§10) — until then, "is the metric map causally traversable?" is answered *not at last_token under linear interchange, and not testable by the framework's geodesic method as the task is parameterized.*
...
- **(CONCLUDED — §6.6 + §6.7)** Causal test of the recovered map, two-part result: (a) **§6.6** — Stage A confirmed the manifold is geometrically metric (chord betweenness, p = 0.0005); the full-residual *linear* interchange swept L20–L31 is a **bounded-negative** (answer attention-reconstructed from the question tokens). (b) **§6.7** — the framework's *geodesic* manifold-steering (its headline method; linear is the degenerate case) is **structurally inapplicable**: `activation_manifold`'s `spline` is locked to `intrinsic_dim == #causal_params` (=1 for categorical `country`) and `flow` is incomplete for the cached-feature/weightless analysis. The framework presupposes a *metric causal parameter*; `country_borders`'s geometry is latent, not parameterized. This pair is the citeable result and the principled "we ran your actual method" answer. Supporting note: an intermediate relocation to the `country` token was also assessed and found degenerate (Stage-A triples yield 0–3/8 usable directions; readout countries geometrically arbitrary), independently confirming `country_borders` is the wrong substrate for a *positive* steering demo.
- **Future experiment (the only route to a positive geodesic result)**: re-parameterize `country_borders` by geographic coordinates — add a `capital_coordinates` (lat, lon) causal variable + identity embedding, mirroring `graph_walk`'s `node_coordinates`. Then `spline` is valid (2 params → intrinsic_dim 2), geodesics are literally geographic, and the "B at the midpoint" hypothesis is directly testable along the framework's native pipeline. This is real task-modeling work (new causal variable, embedding, re-run baseline/subspace, verify counterfactuals) — a deliberate follow-up, not a dangling TODO.
